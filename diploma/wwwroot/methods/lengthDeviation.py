...
def visualisation(matrix, radius):
    ...
    plt.savefig(wwwroot + "\\LDcoordinates\\" + 'particles.png')
    plt.savefig(wwwroot + "\\LDcoordinates\\" + 'particles.svg')
    plt.savefig(wwwroot + "\\LDcoordinates\\" + 'particles.jpeg')
    plt.savefig(wwwroot + "\\LDcoordinates\\" + 'particles.pdf')
    plt.savefig(wwwroot + "\\LDcoordinates\\" + 'particles.eps')

    return "\\" + "LDcoordinates" + "\\" + "particles.svg"

# функция подсчета параметра 
def lengthDeviation(neigh):
...
def heatmap(matrix):
    ...
    plt.savefig(wwwroot + "\\LDcoordinates\\" + 'heatmap.png')
    plt.savefig(wwwroot + "\\LDcoordinates\\" + 'heatmap.svg')
    plt.savefig(wwwroot + "\\LDcoordinates\\" + 'heatmap.pdf')
    plt.savefig(wwwroot + "\\LDcoordinates\\" + 'heatmap.jpeg')
    plt.savefig(wwwroot + "\\LDcoordinates\\" + 'heatmap.eps')

    return "\\" + "LDcoordinates" + "\\" + "heatmap.svg"
...
if (len(unique_r) > 2 or len(unique_r) <= 1):
    ...
    for i in range(len(particles[0])):
        ...
        if (particles[2][i] == r): # если частица основная
            ...
            if (len(temp) != 0 and len(temp) >= 3): output.append((particles[0][i], particles[1][i], lengthDeviation(temp)))

# в случае отсутсвия каких-либо соседей
if (len(output) == 0):
    print("length error")
    sys.exit()
else:
    ...
    with open(wwwroot + "\\" + "LDcoordinates" + "\\" + "heatmapData.txt", 'w') as f:
